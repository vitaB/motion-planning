...
function pointsACl :: "point2d list \<Rightarrow> bool" where 
  "pointsACl xs = True"
|  "pointsACl (a#b#c#xs) = (signedArea a b c > 0 \<and> pointsACl (b#c#xs))"
sorry
function pointsCl :: "point2d list \<Rightarrow> bool" where 
  "pointsCl xs = True"
...
lemma "pointList ( (| xCoord = 0, yCoord = 0 |)#(| xCoord = 2, yCoord = 1 |)#(| xCoord = 3, yCoord = 2 |)#(| xCoord = 1, yCoord = 3 |)#Nil)"
apply (simp add: pointList_def)
done
lemma "L =  [(| xCoord = 0, yCoord = 0 |),(| xCoord = 2, yCoord = 1 |),(| xCoord = 3, yCoord = 2 |),(| xCoord = 1, yCoord = 3 |)]
 \<Longrightarrow> conv_polygon (polygon L)"
 apply (cut_tac P=L in isPolygon)  
 apply (simp add: pointList_def)
 apply (simp add: conv_polygon_def)
 apply (rule disjI2)
 apply (auto simp add: polygon_def pointList_def)
 apply (rule_tac x =L in pointsACl.cases)
 apply (auto)
oops
...
(*wenn ein punkt einer Strecke inside Polygon und ein Punkt einer Strecke, dann gibt es eine intersection*)
